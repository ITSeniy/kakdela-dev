...
- [~] T-104 — Голосовые сообщения (2026-07-04): запись на мобиле (MediaRecorder → webm/opus, обычное вложение через presign/finalize, `files.voice`+`duration_sec`, миграция 0030), компактный плеер `VoiceMessage` на всех клиентах, тач-перемотка в Seekbar. **Код готов, гейты зелёные; остаётся ручной прогон** (телефон → запись → десктоп слушает). Секретные чаты не затронуты (см. карточку).
- [~] T-108 — Hover-превью демки вне комнаты (2026-07-04): скриншот стрима при наведении на стримящего для тех, кто НЕ в его ГС (раньше — только живой трек внутри комнаты). Стример раз в 15 с заливает кадр 480px (`snapshotTrack` с даунскейлом, `screenPreviewUploader`) → Redis TTL 45 с → `GET /voice/:id/screen-preview/:userId`; `ScreenHoverPreview` показывает скриншот как фолбэк/подложку, гейт `canPreview` в ChannelList снят. **Код готов, typecheck + 46 тестов зелёные; остаётся ручной прогон** (A стримит, B вне комнаты наводит → скриншот; B в комнате → живое видео).
- [~] T-107 — Серверные профили как в Discord (2026-07-04): per-server ник и аватар (`server_members.nickname/avatar_url`, миграция 0032). Сервер отдаёт в MemberPublic ЭФФЕКТИВНЫЕ displayName/avatarUrl — клиентский рендер (чат/members/войс) получил ники без правок; поиск/инбокс/реплаи/mention-резолв/LiveKit-имя — COALESCE на бэке; `PATCH /servers/:id/members/me` + WS `member.profile`; модалка «профиль на сервере» в меню сервера (аватар с allowGif из T-106). **Код готов, typecheck + 46 тестов зелёные, миграция применена локально; остаётся ручной прогон** (ник на сервере A ≠ сервер B/DM, live-обновление у второго клиента).
- [~] T-106 — GIF-аватарки, оживающие в войсе (2026-07-04): бэкенд не трогали (gif уже в whitelist, URL с `.gif`), всё на клиенте — `Avatar` получил `animate` + `GifPoster` (статичный первый кадр через canvas-drawImage), тайлы войса и строки ChannelList анимируют по speaking, кроппер принимает GIF без обрезки (`allowGif`). **Код готов, typecheck чистый; остаётся ручной прогон** (GIF-аватар → войс вдвоём → оживает при речи).
- [~] T-105 — Кружки как в Telegram (2026-07-04): та же схема, что T-104 — MediaRecorder (webm vp8+opus, кап 60 с) → вложение с `files.circle` (миграция 0031), общий движок записи `useNoteRecorder`, оверлей с круглым превью, круглый плеер `VideoCircle` (кольцо прогресса) на всех клиентах. **Код готов, гейты зелёные; остаётся ручной прогон** (телефон → кружок → десктоп смотрит).
